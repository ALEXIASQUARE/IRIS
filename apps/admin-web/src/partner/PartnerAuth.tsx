...
export default function PartnerAuth({
  ...
  const [phone, setPhone] = useState('+237691');
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await apiRequest<{ userId: string; devOtp?: string }>('POST', '/auth/register', {
        body: { firstName, lastName, phone, password, countryCode: selectedCountry.isoCode, role: 'PARTNER' },
      });
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    try {
      ...
      try {
        await apiRequest('POST', '/partner/profile', {
          token: res.accessToken,
          body: { currentZoneId: selectedZone.id },
        });
      } catch {
        // profil non créé — le partenaire pourra l'activer plus tard
      }
